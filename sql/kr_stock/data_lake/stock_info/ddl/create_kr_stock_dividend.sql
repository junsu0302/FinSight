...
CREATE TABLE IF NOT EXISTS kr_stock_dividend (
  -- 복합 기본 키 (Composite Primary Key): 동일 종목이 다른 기준일에 여러 배당을 가질 수 있음
  sht_cd TEXT NOT NULL,          -- 종목코드 (API 원본)
  record_date TEXT NOT NULL,     -- 기준일 (API 원본)
  divi_kind TEXT NOT NULL,       -- 배당종류 (API 원본)
